...
function StatCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-card border border-border rounded-2xl p-4 space-y-1">
      ...
      <p className="text-base font-black text-foreground tabular-nums" dir="ltr">
        {value}
      </p>
    </div>
  );
  ...
  return (
    <main className="container mx-auto px-4 py-10 space-y-10" dir="rtl">
      ...
      <nav className="flex items-center gap-1.5 text-xs text-muted-foreground font-bold">
        <Link href="/" className="hover:text-primary-brand transition-colors">الرئيسية</Link>
        <RiArrowLeftSLine size={14} />
        <Link href="/etfs" className="hover:text-primary-brand transition-colors">صناديق الاستثمار</Link>
        <RiArrowLeftSLine size={14} />
      ...
      <div className="pb-8 border-b border-border flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="bg-card border border-border rounded-2xl px-6 py-4 shrink-0 space-y-2 h-fit">
          <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">
            السعر الحالي
          </p>
          ...
          >
            ...
            {item.positive ? "+" : "-"}{item.change.toFixed(2)} ({item.changePercent.toFixed(2)}%)
          </div>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-3xl p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-base font-black text-foreground">الأداء خلال اليوم</h2>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-black text-foreground">أرقام الصندوق</h2>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-card border border-border rounded-3xl p-6 space-y-3">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-black text-foreground">نبذة عن الصندوق</h2>
          </div>
          <p className="text-sm text-muted-foreground leading-loose">
            {item.titleEn} هو صندوق مؤشر متداول يتتبع أداء{" "}
            <span className="font-bold text-foreground">{item.title}</span>.
            يتداول تحت الرمز{" "}
            <span className="font-black text-primary-brand" dir="ltr">{item.ticker}</span>{" "}
            على منصة{" "}
        ...
        <div className="bg-card border border-border rounded-3xl p-6 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-black text-foreground">صناديق مشابهة</h2>
          </div>
          {related.length > 0 ? (
              ...
              {related.map((r) => (
                ...
                >
                  <div className="text-right">
                    ...
                    <p className="text-[10px] text-muted-foreground font-bold uppercase" dir="ltr">
                      {r.ticker}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-black text-foreground tabular-nums" dir="ltr">
                      {r.point.toLocaleString("en-US", { maximumFractionDigits: 2 })}
                    </span>
                    <span
            ...
            <p className="text-sm text-muted-foreground font-medium">لا توجد صناديق مشابهة.</p>
          )}
        </div>
      </div>
    </main>

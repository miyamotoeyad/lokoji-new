...
interface CMCResponse { data: CryptoDetail[] }
...
async function getCryptoDetail(slug: string): Promise<CryptoDetail | undefined> {
...
export default async function CryptoPage({ params }: PageProps) {
  ...
  const stats = [
    { label: "رأس المال السوقي",    value: `$${formatter.format(quote.market_cap)}` },
    { label: "حجم التداول (24س)",   value: `$${formatter.format(quote.volume_24h)}` },
    { label: "العرض المتداول",       value: `${formatter.format(crypto.circulating_supply)} ${crypto.symbol}` },
    { label: "الحد الأقصى للعرض",   value: crypto.max_supply ? formatter.format(crypto.max_supply) : "∞" },
  ];
  ...
  return (
    <main className="container mx-auto px-4 py-8 md:py-10 space-y-6" dir="rtl">
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground font-bold">
        <Link href="/crypto" className="hover:text-primary-brand transition-colors">
          العملات الرقمية
        </Link>
        <RiArrowLeftSLine size={14} className="shrink-0" />
        <span className="text-foreground font-mono uppercase">{crypto.symbol}</span>
      </div>
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <div className="flex items-center gap-2" dir="ltr">
              ...
              <div className={`p-1 rounded-xl ${isNegative ? "bg-destructive/10" : "bg-green-500/10"}`}>
                {isNegative
            ...
            <div className="flex gap-1.5 flex-wrap">
              {changes.map((c) => (
                ...
                >
                  {c.value < 0 ? <RiArrowDownSFill size={11} /> : <RiArrowUpSFill size={11} />}
      ...
      <div className="bg-card border border-border rounded-3xl p-5 md:p-6">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-sm md:text-base font-black text-foreground">الأداء التاريخي</h2>
          </div>
          <span
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-card border border-border rounded-3xl p-5 md:p-6">
          <div className="flex items-center gap-2.5 mb-5">
            ...
            <h2 className="text-sm md:text-base font-black text-foreground">الإحصائيات</h2>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {stats.map((stat) => (
              ...
              >
                ...
                <span className="text-sm md:text-base font-black text-foreground tabular-nums group-hover:text-primary-brand transition-colors block" dir="ltr">
                  {stat.value}
                </span>
              </div>
            ))}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card border border-border rounded-3xl p-5">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <h2 className="text-sm font-black text-foreground">عن {crypto.name}</h2>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed">
              <strong className="text-foreground font-black">{crypto.name}</strong> أصل رقمي رائد.
              القيمة السوقية{" "}
          ...
          <div className="dark:bg-card bg-dprimary rounded-3xl p-5 text-white relative overflow-hidden">
            ...
            <RiLineChartLine className="absolute -bottom-4 -right-4 text-white/5 pointer-events-none" size={80} />

...
function buildStats(item: WorldMarketItem) {
  const c = item.currency || "";
  return [
    { group: "بيانات السعر", label: "سعر الافتتاح",       value: item.openPrice  > 0 ? `${fmt(item.openPrice)} ${c}`  : "—" },
    { group: "بيانات السعر", label: "أعلى سعر اليوم",     value: item.dayHigh    > 0 ? `${fmt(item.dayHigh)} ${c}`    : "—" },
    { group: "بيانات السعر", label: "أدنى سعر اليوم",     value: item.dayLow     > 0 ? `${fmt(item.dayLow)} ${c}`     : "—" },
    { group: "بيانات السعر", label: "إغلاق أمس",          value: item.prevClose  > 0 ? `${fmt(item.prevClose)} ${c}`  : "—" },
    { group: "التداول",      label: "حجم التداول",         value: item.volume     > 0 ? item.volume.toLocaleString("en-US") : "—" },
    { group: "النطاق السنوي", label: "الأعلى 52 أسبوع",   value: item.weekHigh52 > 0 ? `${fmt(item.weekHigh52)} ${c}` : "—" },
    { group: "النطاق السنوي", label: "الأدنى 52 أسبوع",   value: item.weekLow52  > 0 ? `${fmt(item.weekLow52)} ${c}`  : "—" },
    { group: "معلومات",      label: "رمز المؤشر",          value: item.ticker },
    { group: "معلومات",      label: "التغيير",              value: `${item.positive ? "+" : ""}${item.change.toFixed(2)}` },
    { group: "معلومات",      label: "نسبة التغيير",        value: `${Math.abs(item.changePercent).toFixed(2)}%` },
  ];
}
...
export default async function WorldMarketSlugPage({ params }: { params: Params }) {
  ...
  const item = allIndices.find((i) => i.slug === slug);
    ...
    <main className="container mx-auto px-4 py-8 md:py-10 space-y-6" dir="rtl">
      ...
      <div className="bg-card border border-border rounded-3xl p-5 md:p-6">
        ...
        <div className="h-48 sm:h-56 md:h-72 w-full">
          <WorldMarketChart data={candleData} isPositive={item.positive} ticker={item.ticker} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-card border border-border rounded-3xl p-5 md:p-6">
          ...
          {/* ── 52-week range bar ── */}
          {item.weekHigh52 > 0 && item.weekLow52 > 0 && item.weekHigh52 !== item.weekLow52 && (
            <div className="mt-4 bg-muted/50 border border-border rounded-2xl p-4 space-y-2">
              ...
              <div className="relative h-2 bg-muted rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min(Math.max(((item.price - item.weekLow52) / (item.weekHigh52 - item.weekLow52)) * 100, 0), 100)}%`,
                  }}
                />
                <div
                  className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-primary-brand border-2 border-card shadow"
                  style={{
                    left: `calc(${Math.min(Math.max(((item.price - item.weekLow52) / (item.weekHigh52 - item.weekLow52)) * 100, 0), 100)}% - 6px)`,
                  }}
                />
              </div>
              <div className="flex justify-between text-[10px] text-muted-foreground font-bold" dir="ltr">
                <span>أدنى {fmt(item.weekLow52)}</span>
                <span className="text-primary-brand font-black">{fmt(item.price)}</span>
                <span>أعلى {fmt(item.weekHigh52)}</span>
              </div>
            </div>
          )}
        </div>

...
interface Props {
  item: ETFItem;
  initialData: CandlePoint[]; // 1d preloaded server-side
}
...
function CustomTooltip({
  ...
  return (
    <div
      className="bg-card border border-border rounded-xl px-3 py-2 shadow-lg text-xs font-black"
      dir="ltr"
    >
...
export default function ETFChart({ item, initialData }: Props) {
  ...
  async function handleRangeChange(newRange: ChartRange) {
    ...
    setLoading(true);
    try {
      // ✅ fetch on the client for range switching — hits Next.js cache
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span
          className={`mr-auto inline-flex items-center gap-1.5 text-[10px] font-black px-2.5 py-1 rounded-full border ${
            isLive
        ...
        >
          <span
            className={`w-1.5 h-1.5 rounded-full ${isLive ? "bg-green-500 animate-pulse" : "bg-muted-foreground"}`}
          />
      ...
      {/* ── Chart ── */}
      <div
        className={`w-full h-64 transition-opacity duration-300 ${loading ? "opacity-40 pointer-events-none" : "opacity-100"}`}
      >
        {!isLive ? (

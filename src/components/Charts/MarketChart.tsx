...
function CustomTooltip({
  ...
  return (
    <div
      className="bg-card border border-border rounded-xl px-3 py-2 shadow-lg text-xs font-black space-y-0.5"
      dir="ltr"
    >
  ...
  async function handleRangeChange(newRange: ChartRange) {
    ...
    try {
      const res = await fetch(
        `/api/candles?ticker=${encodeURIComponent(ticker)}&range=${newRange}`,
      );
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {/* Live badge — shrink-safe */}
        <span
          className={`shrink-0 inline-flex items-center gap-1 text-[10px] font-black px-2 py-1 rounded-full border ${
            isLive
        ...
        >
          <span
            className={`w-1.5 h-1.5 rounded-full shrink-0 ${isLive ? "bg-green-500 animate-pulse" : "bg-muted-foreground"}`}
          />
      ...
      {/* ── Chart ── */}
      <div
        className={`w-full h-48 sm:h-56 md:h-64 transition-opacity duration-300 ${
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <XAxis
                ...
                // show fewer ticks on mobile via tickCount
                tickCount={4}
              />
